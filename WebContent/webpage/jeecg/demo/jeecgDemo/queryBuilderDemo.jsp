...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding: 1px;">
		<t:datagrid name="jeecgDemoList2" title="高级查询示例"
			actionUrl="jeecgDemoController.do?datagrid" idField="id"
			...
			<t:dgCol title="生日" field="birthday" formatter="yyyy/MM/dd"
				query="true"></t:dgCol>
			<t:dgToolBar operationCode="edit" title="录入" icon="icon-edit"
				url="jeecgDemoController.do?addorupdate" funname="add"></t:dgToolBar>
			<t:dgToolBar operationCode="edit" title="编辑" icon="icon-edit"
				url="jeecgDemoController.do?addorupdate" funname="update"></t:dgToolBar>
			<t:dgToolBar operationCode="add" title="高级查询" icon="icon-search"
				funname="queryBuilder"></t:dgToolBar>
		</t:datagrid>
	</div>
</div>
<div style="position:relative;overflow:auto;">
	<div id="w" class="easyui-window"
		data-options="closed:true,title:'高级查询构造器'"
		style="width:600px;height:370px;padding:0px">
		<div class="easyui-layout" data-options="fit:true">
         ...
         onAfterEdit:function(node){  
			...
			<div data-options="region:'center'" style="padding:0px;">
				<table id="tg" class="easyui-treegrid" title="查询条件编辑"
			...
			">
					<thead>
						<tr>
							<th
								data-options="field:'relation',width:19,formatter:function(value,row){
							...
							<th
								data-options="field:'field',width:30,formatter:function(value,row){
								var data=  
								[  
							...
							<th
								data-options="field:'condition',width:20,align:'right',formatter:function(value,row){
								var data=  
								[  
							...
							data:  
								...
								{'conditionId':'<','conditionName':'小于'},
								{'conditionId':'like','conditionName':'包含'},
								{'conditionId':'in','conditionName':'在其中'} ], required:true
								}}">条件</th>
							<th data-options="field:'value',width:30,editor:'text'">值</th>
							<th
								data-options="field:'opt',width:30,formatter:function(value,row){
			...
			<div data-options="region:'south',border:false"
				...
				<a class="easyui-linkbutton" data-options="iconCls:'icon-ok'"
					href="javascript:void(0)" onclick="javascript:queryBuilderSearch()">OK</a>
				<a class="easyui-linkbutton" data-options="iconCls:'icon-cancel'"
					href="javascript:void(0)"
					onclick="javascript:$('#w').window('close')">Cancel</a>
			</div>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	$(document).ready(
			function() {
				append();
				$("input[name='createDate_begin']").attr("class", "Wdate")
				...
				$("input[name='createDate_end']").attr("class", "Wdate").attr(
						"style", "height:20px;width:90px;").click(function() {
					WdatePicker({
						dateFmt : 'yyyy-MM-dd'
					});
				});
				$("input[name='birthday']").attr("class", "Wdate").attr(
						"style", "height:20px;width:90px;").click(function() {
					WdatePicker({
						dateFmt : 'yyyy-MM-dd'
					});
	...
	function historyQuery(index) {
		var data = {
			rows : JSON.parse(his[index].json)
		};
		var t = $('#tg');
