...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:1px;">
		<t:datagrid name="zformPersonList" checkbox="true" fitColumns="false" title="人员基本信息数据库" actionUrl="zformPersonController.do?datagrid&status=1" idField="id" fit="true" queryMode="group">
			...
			<t:dgToolBar title="编辑" icon="icon-edit" url="zformPersonController.do?goUpdate&status=8" funname="update"></t:dgToolBar>
			<t:dgToolBar title="批量删除" icon="icon-remove" url="zformPersonController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
			<t:dgToolBar title="查看" icon="icon-search" url="zformPersonController.do?goUpdate&status=8" funname="detail"></t:dgToolBar>
			<t:dgToolBar title="导出"         icon="icon-putout" funname="ExportXls"></t:dgToolBar>
			<t:dgToolBar title="个人调动记录台帐" icon="icon-putout" funname="ExportPersonalResumeWithInnerTransfer"></t:dgToolBar>
			<t:dgToolBar title="内部调动" icon="pictures" url="zformPersonController.do?goTransferInternal" funname="update"></t:dgToolBar>
...
function ExportPersonalResumeWithInnerTransfer(title, url, id){
	var rowData = $('#' + id).datagrid('getSelected');
	if (!rowData) {
		tip('请选择生成台帐的人员');
		return;
	}		
	JeecgExcelExport("zformPersonController.do?exportPersonalResumeWithInnerTransfer&id="+rowData['id'], "zformPersonList");		
}
