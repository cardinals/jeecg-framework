...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:1px;">
		<t:datagrid name="zformPersonList" checkbox="true" fitColumns="false" title="人员基本信息数据库" actionUrl="zformPersonController.do?datagrid&status=4" idField="id" fit="true" queryMode="group">
			...
			<t:dgToolBar title="录入" icon="icon-add" url="zformPersonController.do?goAdd&status=4" funname="add"></t:dgToolBar>
			<t:dgToolBar title="编辑" icon="icon-edit" url="zformPersonController.do?goUpdate&status=4" funname="update"></t:dgToolBar>
			<t:dgToolBar title="批量删除" icon="icon-remove" url="zformPersonController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
			<t:dgToolBar title="查看" icon="icon-search" url="zformPersonController.do?goUpdate&status=4" funname="detail"></t:dgToolBar>
			<t:dgToolBar title="导出" icon="icon-putout" funname="ExportXls"></t:dgToolBar>			
			<t:dgToolBar title="转入调出库" icon="pictures" url="zformPersonController.do?doTo_TransferExternalAccept" funname="doTo_TransferExternalAccept"></t:dgToolBar>
			<t:dgToolBar title="退回在职库" icon="pictures" url="zformPersonController.do?doTo_TransferExternalRevoke" funname="doTo_TransferExternalRevoke"></t:dgToolBar>
		</t:datagrid>
	</div>
</div>
...
<script type="text/javascript">
 $(document).ready(function(){
 	...
 	function doTo_TransferExternalAccept(title,url,id){
 		var rowData = $('#'+id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择转入调出库项目');
			return;
 	...
 	function doTo_TransferExternalRevoke(title,url,id){
 		var rowData = $('#'+id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择转回在职库项目');
			return;
...
//导出
function ExportXls() {
	JeecgExcelExport("zformPersonController.do?exportXls&currentStatus=4","zformPersonList");
}
