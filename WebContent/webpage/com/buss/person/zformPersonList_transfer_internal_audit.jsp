...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:1px;">
		<t:datagrid name="zformPersonList" checkbox="true" fitColumns="false" title="人员基本信息数据库" actionUrl="zformPersonController.do?datagrid&status=9" idField="id" fit="true" queryMode="group">
			...
			<t:dgToolBar title="编辑" icon="icon-edit" url="zformPersonController.do?goUpdate&status=9" funname="update"></t:dgToolBar>
			<t:dgToolBar title="批量删除" icon="icon-remove" url="zformPersonController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
			<t:dgToolBar title="查看" icon="icon-search" url="zformPersonController.do?goUpdate&status=9" funname="detail"></t:dgToolBar>
			<t:dgToolBar title="导出" icon="icon-putout" funname="ExportXls"></t:dgToolBar>
			<t:dgToolBar title="开具商调函" icon="icon-putout" funname="GenerateInnerTransferBusinessDocument"></t:dgToolBar>
			<t:dgToolBar title="开具调干通知书" icon="icon-putout" funname="GenerateInnerTransferNotifyDocument"></t:dgToolBar>
			<t:dgToolBar title="开具干部介绍信" icon="icon-putout" funname="GenerateInnerTransferReferenceDocument"></t:dgToolBar>
			<t:dgToolBar title="人员调动批准" icon="pictures" url="zformPersonController.do?doTo_TransferInternalAccept" funname="doTo_TransferInternalAccept"></t:dgToolBar>
			<t:dgToolBar title="人员调动撤销" icon="pictures" url="zformPersonController.do?doTo_TransferInternalRevoke" funname="doTo_TransferInternalRevoke"></t:dgToolBar>
		</t:datagrid>
	</div>
</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		//给时间控件加上样式
		$("#zformPersonListtb").find("input[name='birthday']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
			WdatePicker({
				dateFmt : 'yyyy-MM-dd'
			});
		});
		$("#zformPersonListtb").find("input[name='startWorkDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
			WdatePicker({
				dateFmt : 'yyyy-MM-dd'
			});
		});
		$("#zformPersonListtb").find("input[name='graduateDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
			WdatePicker({
				dateFmt : 'yyyy-MM-dd'
			});
		});
		$("#zformPersonListtb").find("input[name='enterDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
			WdatePicker({
				dateFmt : 'yyyy-MM-dd'
			});
		});
		$("#zformPersonListtb").find("input[name='juniorTitleDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
			WdatePicker({
				dateFmt : 'yyyy-MM-dd'
			});
		});
		$("#zformPersonListtb").find("input[name='secondaryTitleDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
			WdatePicker({
				dateFmt : 'yyyy-MM-dd'
			});
		});
		$("#zformPersonListtb").find("input[name='seniorTitleDate']").attr("class", "Wdate").attr("style", "height:20px;width:90px;").click(function() {
			WdatePicker({
				dateFmt : 'yyyy-MM-dd'
			});
		});
	...
	function doTo_TransferInternalAccept(title, url, id) {
		var rowData = $('#' + id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择人员调动项目');
			return;
	...
	function doTo_TransferInternalRevoke(title, url, id) {
		var rowData = $('#' + id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择撤销的人员调动项目');
			return;
	...
	function GenerateInnerTransferBusinessDocument(title, url, id) {
		var rowData = $('#' + id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择开具商调函的项目');
			return;
		}		
		JeecgExcelExport("zformPersonController.do?generateInnerTransferBusinessDocument&id="+rowData['id'], "zformPersonList");		
	}
	
	function GenerateInnerTransferNotifyDocument(title, url, id) {
		var rowData = $('#' + id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择开具调干通知书的项目');
			return;
		}		
		JeecgExcelExport("zformPersonController.do?generateInnerTransferNotifyDocument&id="+rowData['id'], "zformPersonList");		
	}
	
	function GenerateInnerTransferReferenceDocument(title, url, id) {
		var rowData = $('#' + id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择开具干部介绍信的项目');
			return;
		}		
		JeecgExcelExport("zformPersonController.do?generateInnerTransferReferenceDocument&id="+rowData['id'], "zformPersonList");		
	}
	...
	//导出
	function ExportXls() {
		JeecgExcelExport("zformPersonController.do?exportXls&currentStatus=9", "zformPersonList");
	}
