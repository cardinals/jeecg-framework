...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:1px;">
		<t:datagrid name="zformPersonList" checkbox="true" fitColumns="false" title="人员基本信息数据库" actionUrl="zformPersonController.do?datagrid&status=3" idField="id" fit="true" queryMode="group">
			...
			<t:dgToolBar title="录入" icon="icon-add" url="zformPersonController.do?goAdd&status=3" funname="add"></t:dgToolBar>
			<t:dgToolBar title="编辑" icon="icon-edit" url="zformPersonController.do?goUpdate&status=3" funname="update"></t:dgToolBar>
			<t:dgToolBar title="批量删除" icon="icon-remove" url="zformPersonController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
			<t:dgToolBar title="查看" icon="icon-search" url="zformPersonController.do?goUpdate&status=3" funname="detail"></t:dgToolBar>			
			...
			<t:dgToolBar title="转回在职库" icon="pictures" url="zformPersonController.do?doTo_Active" funname="doTo_Active"></t:dgToolBar>
		</t:datagrid>
	</div>
</div>
...
<script type="text/javascript">
 $(document).ready(function(){
 	...
 	function doTo_Active(title,url,id){
 		var rowData = $('#'+id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择转入在职库项目');
			return;
...
//导出
function ExportXls() {
	JeecgExcelExport("zformPersonController.do?exportXls&currentStatus=3","zformPersonList");
}
