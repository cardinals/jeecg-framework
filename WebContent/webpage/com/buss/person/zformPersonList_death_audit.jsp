...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:1px;">
		<t:datagrid name="zformPersonList" checkbox="true" fitColumns="false" title="人员基本信息数据库" actionUrl="zformPersonController.do?datagrid&status=6" idField="id" fit="true" queryMode="group">
			...
			<t:dgToolBar title="录入" icon="icon-add" url="zformPersonController.do?goAdd&status=6" funname="add"></t:dgToolBar>
			<t:dgToolBar title="编辑" icon="icon-edit" url="zformPersonController.do?goUpdate&status=6" funname="update"></t:dgToolBar>
			<t:dgToolBar title="批量删除" icon="icon-remove" url="zformPersonController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
			<t:dgToolBar title="查看" icon="icon-search" url="zformPersonController.do?goUpdate&status=6" funname="detail"></t:dgToolBar>			
			<t:dgToolBar title="导出" icon="icon-putout" funname="ExportXls"></t:dgToolBar>			
			<t:dgToolBar title="死亡人员丧抚费审批表" icon="icon-putout" funname="ExportDeathExpenseAudit"></t:dgToolBar>			
			<t:dgToolBar title="转入死亡库" icon="pictures" url="zformPersonController.do?doTo_DeathAccept" funname="doTo_DeathAccept"></t:dgToolBar>
			<t:dgToolBar title="退回在职库" icon="pictures" url="zformPersonController.do?doTo_DeathRevoke_ToActive" funname="doTo_DeathRevoke_ToActive"></t:dgToolBar>
			<t:dgToolBar title="退回退休库" icon="pictures" url="zformPersonController.do?doTo_DeathRevoke_ToRetire" funname="doTo_DeathRevoke_ToRetire"></t:dgToolBar>
		</t:datagrid>
	</div>
</div>
...
<script type="text/javascript">
 $(document).ready(function(){
 	...
 	function doTo_DeathAccept(title,url,id){
 		var rowData = $('#'+id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择转入死亡项目');
			return;
 	...
 	function doTo_DeathRevoke_ToActive(title,url,id){
 		var rowData = $('#'+id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择退回在职项目');
			return;
 	...
 	function doTo_DeathRevoke_ToRetire(title,url,id){
 		var rowData = $('#'+id).datagrid('getSelected');
		if (!rowData) {
			tip('请选择退回退休项目');
			return;
...
function ExportXls() {
	...
	JeecgExcelExport("zformPersonController.do?exportXls&currentStatus=6","zformPersonList");
}

//导出死亡人员花名册
function ExportDeathExpenseList() {
	JeecgExcelExport("zformPersonController.do?exportDeathExpenseList","zformPersonList");
}

//导出死亡人员审批表
function ExportDeathExpenseAudit(title,url,id){
	var rowData = $('#'+id).datagrid('getSelected');
	if (!rowData) {
		tip('请选择项目');
		return;
	}
	JeecgExcelExport("zformPersonController.do?exportDeathExpenseAudit&id="+rowData['id'],"zformPersonList");
}
