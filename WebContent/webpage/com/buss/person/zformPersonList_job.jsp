...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:1px;">
		<t:datagrid name="zformPersonList" checkbox="true" fitColumns="false" title="人员基本信息数据库" actionUrl="zformPersonController.do?datagrid&status=1" idField="id" fit="true" queryMode="group">
			...
			<t:dgToolBar title="编辑" icon="icon-edit" url="zformPersonController.do?goUpdate&status=10" funname="update"></t:dgToolBar>
			<t:dgToolBar title="批量删除" icon="icon-remove" url="zformPersonController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
			<t:dgToolBar title="查看" icon="icon-search" url="zformPersonController.do?goUpdate&status=10" funname="detail"></t:dgToolBar>
			<t:dgToolBar title="导出" icon="icon-putout" funname="ExportXls"></t:dgToolBar>
			<t:dgToolBar title="个人历年岗位聘用情况" icon="icon-putout" funname="ExportPersonalResumeWithJob"></t:dgToolBar>
			<t:dgToolBar title="汇总表" icon="icon-putout" url="zformPersonController.do?exportJobStatSum1" funname="ExportJobStatSum1"></t:dgToolBar>
			<t:dgToolBar title="聘用结果备案表" icon="icon-putout" url="zformPersonController.do?exportJobStatSum2" funname="ExportJobStatSum2"></t:dgToolBar>
			<t:dgToolBar title="聘用备案名册" icon="icon-putout" url="zformPersonController.do?doExportJobStatDetails" funname="ExportJobStatDetails"></t:dgToolBar>			
			<t:dgToolBar title="岗位聘用登记" icon="pictures" url="zformPersonController.do?goTo_Job" funname="update"></t:dgToolBar>
		</t:datagrid>
	</div>
</div>
...
<script type="text/javascript">
 $(document).ready(function(){
 	...
 	function ExportPersonalResumeWithJob(title, url, id) {
 		var rowData = $('#' + id).datagrid('getSelected');
 		if (!rowData) {
 			tip('请选择生成个人历年岗位聘用台账的人员');
 			return;
 		}		
 		JeecgExcelExport("zformPersonController.do?exportPersonalResumeWithJob&id="+rowData['id'], "zformPersonList");		
 	}
...
function ExportJobStatSum1(){
	JeecgExcelExport("zformPersonController.do?exportJobStatSum1", "zformPersonList");
}

function ExportJobStatSum2(){
	JeecgExcelExport("zformPersonController.do?exportJobStatSum2", "zformPersonList");
}

function ExportJobStatDetails(){
	
	JeecgExcelExport("zformPersonController.do?exportJobStatDetails", "zformPersonList");
}
